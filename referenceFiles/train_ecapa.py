...
#        print(self.model.load_state_dict(state_dict['model'], strict=False))
        ...
        #self.load_state_dict(torch.load("/home/iiitdwd/cocosda_wavlm/exps/exp1_wavlm3/epoch=3-VEER=17.600-mindcf=0.751.ckpt")['state_dict'],strict=False)
        import math 
        ...
        self.load_state_dict(torch.load("/home/iiitdwd/cocosda_wavlm/exps/exp_17thMay_step2/epoch=3-VEER=49.890-mindcf=1.000.ckpt",weights_only=False)['state_dict'],strict=False)
        self.save_hyperparameters()
        #self.loss_function = LossFunction(
        #     kwargs['nOut'], kwargs['nClasses'], margin=kwargs['margin'], scale=kwargs['scale']
        # )
        #self.loss_function = losses.MultiSimilarityLoss(alpha=2, beta=50, base=0.5,)
    def forward(self, x):
    ...
    def configure_optimizers(self):
        ...
        return optimizer
    def evaluate(self):
        all_scores, all_labels, all_trials=evaluateFromList(self.model, self.hparams.test_list, "/home/iiitdwd/cocosda_wavlm/", self.hparams.nDataLoaderThread,eval_frames=self.hparams.eval_frames)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--max_epoch',      type=int,   default=2,    help='Maximum number of epochs')
        ...
        parser.add_argument('--nLanguage',       type=int,   default=1,   help='Number of language in batch')
        parser.add_argument('--nClasses',       type=int,   default=100,   help='Number of speakers in the softmax layer, only for softmax-based losses')
        ...
        parser.add_argument('--save_path',      type=str,   default="exps/exp_17thMay_step3", help='Path for model and logs')

        ## Training and test data
        parser.add_argument('--train_list',     type=str,   default="train_17thMay.txt",  help='Train list')
        parser.add_argument('--test_list',      type=str,   default="test_17thMay_formatted.txt",   help='Evaluation list')
        parser.add_argument('--musan_path',     type=str,   default="/home/iiitdwd/kaldiSpace/database/musan_split", help='Absolute path to the test set')
        parser.add_argument('--rir_path',       type=str,   default="/home/iiitdwd/kaldiSpace/database/rirs_noises/RIRS_NOISES/simulated_rirs", help='Absolute path to the test set')
...
trainer = pl.Trainer( max_epochs=2,log_every_n_steps =200,accelerator='gpu',check_val_every_n_epoch =1,callbacks=[ScoreCallback(),pl.callbacks.ModelCheckpoint(args.save_path,filename ="{epoch}-{VEER:.3f}-{mindcf:.3f}",save_last =True,mode ='min',monitor ="VEER",save_on_train_epoch_end =True)])
trainer.fit(model=model.to(device="cuda"))
