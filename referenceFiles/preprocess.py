PATH_DATA= "/home/iiitdwd/kaldiSpace/database/IITG-MV"
import glob,os,sys
...
def read_and_split(xs):
  ...
  for path in tqdm(xs):
    ...
    for index,step in enumerate(range(0,len(data),chunk_size)):
        ...
        new_path=os.path.join("/home/iiitdwd/cocosda_wavlm/data/split_ds/dev_data",new_path)
        if audio.shape[0] / samplerate < 1.:
          continue
        sf.write(new_path, audio, samplerate)
  print("Done process")
    # this does not work
xss=list(glob.glob(f"{PATH_DATA}/I_MSV_DEV_ENR/Dev_data_org/*.wav"))
...
def read_and_split1(xs):
  ...
  for path in tqdm(xs):
    ...
    for index,step in enumerate(range(0,len(data),chunk_size)):
        audio = data[step:step+chunk_size]
        new_path = os.path.basename(path).replace(".wav",f"_{index}.wav")
        new_path=os.path.join("/home/iiitdwd/cocosda_wavlm/data/split_ds/env_data",new_path)
        if audio.shape[0] / samplerate < 1.:
          continue
        sf.write(new_path, audio, samplerate)
xss=list(glob.glob(f"{PATH_DATA}/I_MSV_DEV_ENR/Enr_data/*.wav"))
processes = [multiprocessing.Process(target=read_and_split1, args=(x,)) for x in np.array_split(xss, 1)]
[p.start() for p in processes]
result = [p.join() for p in processes]
print(result)
