...
class SpeakerLightingModule(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        return optimizer
    def evaluate(self):
        all_scores, all_labels, all_trials=evaluateFromList(self.model, self.hparams.test_list, "/home/iiitdwd/cocosda_wavlm/data/split_ds", self.hparams.nDataLoaderThread,eval_frames=self.hparams.eval_frames)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--trainfunc',      type=str,   default="aam_inter",     help='Loss function')
        ...
        parser.add_argument('--train_list',     type=str,   default="train.txt",  help='Train list')
        parser.add_argument('--test_list',      type=str,   default="test.txt",   help='Evaluation list')
...
import glob,os,sys
import pandas as pd 
list_wavs = list(glob.glob("data/split_ds/env_data/*.wav"))
list_wavs += list(glob.glob("data/split_ds/dev_data/*.wav"))
list_wavs += list(glob.glob("data/split_ds/augment/*.wav"))
data=pd.DataFrame({"path":list_wavs})
data['speaker'] = data.path.apply(lambda x:os.path.basename(x).split("_")[0])
data['lang'] = data.path.apply(lambda x:os.path.basename(x).split("_")[1][1:3])
with open("train.txt","w+") as f:
  for index,row in data.iterrows():
    f.write(f"{row.speaker} {row.path} {row.lang}\n")
#test=pd.read_csv("split_ds/imsv-public-test/imsv-public-test/public_test_cohart.csv")

parser = SpeakerLightingModule.add_model_specific_args(parser)
args = parser.parse_args()
print(vars(args))
model =SpeakerLightingModule(**vars(args))
trainer = pl.Trainer( max_epochs=100,log_every_n_steps =200,accelerator='gpu',check_val_every_n_epoch =1,callbacks=[ScoreCallback(),pl.callbacks.ModelCheckpoint(args.save_path,filename ="{epoch}-{VEER:.3f}-{mindcf:.3f}",save_last =True,mode ='min',monitor ="VEER",save_on_train_epoch_end =True)])
